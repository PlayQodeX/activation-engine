...
## Dispatch — when invoked as `/activate [args]`

Read the first argument and run the matching script, then act on its output.
No args → the workspace bootstrap (`activate.mjs` for cwd).

| Args | Run |
|------|-----|
| `help` / `--help` | `node "<SKILL>/activate.mjs" --help` (print the guide verbatim) |
| *(none)* or a path | `node "<SKILL>/activate.mjs" [path]` |
| `scan …` | `node "<SKILL>/scan.mjs" …` |
| `list` | `node "<SKILL>/activate.mjs" --list` |
| `instances` / `list instances` | `node "<SKILL>/instance.mjs" list [--grep …]` |
| `default` | `node "<SKILL>/activate.mjs" --default` |
| `create`/`add`/`remove`/`rename`/`delete`/`default`/`export`/`import`/`sync` … | `node "<SKILL>/instance.mjs" <verb> …` |
| a known instance name, or `--instance <name>` | `node "<SKILL>/activate.mjs" --instance <name>` |

The read-only verbs are also first-class slash commands (generated by `sync`):
**`/activate-help`**, **`/activate-scan`**, **`/activate-list`**,
**`/activate-instances`**, plus **`/activate-<slug>`** per instance. Run
`instance.mjs sync` once (or any instance mutation) to (re)generate them.
...
| **instance from-active** | `node "<SKILL>/instance.mjs" create <name> --from-active [--path <dir>] [--include global,plugins,global-rules\|all]` | Seed a bundle from the folder's live resolved stack. Workspace-specific by default; `--include` also captures ambient global/plugin skills + global rules. |
